...
--   https://supabase.com/dashboard/project/hawagqifmushkxlusqii/sql/new
-- ============================================================
...
-- User preferences
-- NOTE: Column names match what settings/page.tsx reads/writes.
CREATE TABLE IF NOT EXISTS public.user_preferences (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL UNIQUE REFERENCES auth.users(id) ON DELETE CASCADE,
  work_duration_mins INTEGER DEFAULT 25,
  ...
  energy_pattern TEXT DEFAULT 'morning',      -- 'morning', 'afternoon', 'evening', 'night'
  work_hours_start TEXT DEFAULT '09:00',      -- stored as HH:MM string
  work_hours_end TEXT DEFAULT '17:00',        -- stored as HH:MM string
...
CREATE INDEX IF NOT EXISTS idx_topic_reviews_topic_id ON public.topic_reviews(topic_id);
...
-- Profiles
DROP POLICY IF EXISTS "Profiles are viewable by the user who owns it" ON public.profiles;
...
-- Tasks
DROP POLICY IF EXISTS "Tasks are viewable by the owner" ON public.tasks;
...
-- Task sessions
DROP POLICY IF EXISTS "Sessions are viewable by the owner" ON public.task_sessions;
CREATE POLICY "Sessions are viewable by the owner"
  ON public.task_sessions FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own sessions" ON public.task_sessions;
CREATE POLICY "Users can insert their own sessions"
  ON public.task_sessions FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own sessions" ON public.task_sessions;
CREATE POLICY "Users can update their own sessions"
  ON public.task_sessions FOR UPDATE
  USING (auth.uid() = user_id);

-- Topics
DROP POLICY IF EXISTS "Topics are viewable by the owner" ON public.topics;
...
-- Topic reviews
DROP POLICY IF EXISTS "Reviews are viewable by the owner" ON public.topic_reviews;
CREATE POLICY "Reviews are viewable by the owner"
  ON public.topic_reviews FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own reviews" ON public.topic_reviews;
CREATE POLICY "Users can insert their own reviews"
  ON public.topic_reviews FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Schedule blocks
DROP POLICY IF EXISTS "Schedule blocks are viewable by the owner" ON public.schedule_blocks;
CREATE POLICY "Schedule blocks are viewable by the owner"
  ON public.schedule_blocks FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own schedule blocks" ON public.schedule_blocks;
...
-- AI insights
DROP POLICY IF EXISTS "Insights are viewable by the owner" ON public.ai_insights;
...
-- Auto-create profile + preferences on new user signup
...
DROP TRIGGER IF EXISTS update_task_sessions_updated_at ON public.task_sessions;
...
-- Handle task completion (auto-set completed_at)
...
-- Handle session end (auto-calculate duration)
...
DROP TRIGGER IF EXISTS on_session_end ON public.task_sessions;
